...
static int rbppc_nand_parse_partitions(struct rbppc_nand_prv *prv, struct mtd_partition **partitions, int *partition_count)
{
...
#ifdef CONFIG_MTD_CMDLINE_PARTS
	retval = parse_mtd_partitions(&prv->mtd, rbppc_nand_pprobes, partitions, 0);
	if (retval < 0)
		return retval;
	else if (retval > 0) {
		*partition_count = retval;
		return 0;
	}
#endif
#ifdef CONFIG_MTD_OF_PARTS
	dn_partitions = of_get_next_child(prv->dev->of_node, NULL);

	retval = of_mtd_parse_partitions(prv->dev, dn_partitions, partitions);
...
static __devinit int rbppc_nand_probe(struct platform_device *pdev)
{
	...
	retval = rbppc_nand_parse_partitions(prv, &partitions, &partition_count);
	if (retval) {
